...
|---------|------|------|----------|--------|----------------|------|
| **PrematchCommentAPI** | Data & Infrastructure Layer - API Layer | 後端 endpoint 定義（評論相關） | 1. `GET /{TBD 個人資訊 API}`<br>2. `GET /chat/match/comment/info/{refId}`<br>3. `GET /chat/match/comment/popular`<br>4. `GET /chat/match/comment/newest`<br>5. `GET /chat/match/comment/replies`<br>6. `POST /chat/match/comment`<br>7. `POST /chat/match/comment/like` | PrematchCommentClient | PrematchCommentFeature | 1. 個人資訊 API 目前為待定（TBD） |
| **ChatAPI** | Data & Infrastructure Layer - API Layer | 後端 endpoint 定義（聊天室相關） | 1. `GET /chat/match/{refId}`<br>2. `POST /chat/match/join`<br>3. `POST /chat/match/backward`<br>4. `POST /chat/match/leave`<br>5. `POST /chat/{matchId}/message`<br>6. `wss://www.encorebet.net/chat/websocket/web-chat`（WebSocket） | LiveChatClient<br>ChatWebSocketClient | LiveChatFeature | 1. WebSocket 端點用於即時訊息傳遞 |
